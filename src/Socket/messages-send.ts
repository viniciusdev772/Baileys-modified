...
export const makeMessagesSocket = (config: SocketConfig) => {
	...
	// Patch para garantir que botões e listas funcionem em todas as plataformas
	const patchMessageRequiresBeforeSending = (msg: proto.IMessage, recipientJids: string[]): proto.IMessage => {
		// Fix listMessage para funcionar em celular e PC
	...
	const createParticipantNodes = async (
		...
		// Aplica patch adicional para botões e listas funcionarem em todas plataformas
		patched = patchMessageRequiresBeforeSending(patched as proto.IMessage, recipientJids)
	...
	const relayMessage = async (
		...
		await authState.keys.transaction(async () => {
			...
			// Adiciona nó biz/interactive para mensagens de botões (igual baileys-pro/itsukichann)
			const normalizedMsg = normalizeMessageContent(message)
			const msgContentType = normalizedMsg ? getContentType(normalizedMsg) : null
			if (!isNewsletter && (msgContentType === 'interactiveMessage' || msgContentType === 'buttonsMessage')) {
				// Verifica se é um botão de pagamento (payment_info ou review_and_pay)
				const nativeFlow = normalizedMsg?.interactiveMessage?.nativeFlowMessage
				const firstButtonName = nativeFlow?.buttons?.[0]?.name

				let bizNode: BinaryNode

				if (firstButtonName === 'payment_info' || firstButtonName === 'review_and_pay') {
					// Formato especial para botões de pagamento (itsukichann style)
					bizNode = {
						tag: 'biz',
						attrs: {
					...
					logger.debug({ jid: destinationJid }, `adding biz node for ${firstButtonName}`)
				} else {
					// Formato padrão para outros botões interativos
					bizNode = {
						tag: 'biz',
						attrs: {},
						content: [{
					...
					logger.debug({ jid: destinationJid }, 'adding biz/interactive node for buttons')
				}
				;(stanza.content as BinaryNode[]).push(bizNode)
			}

			// Adiciona nó biz/list para mensagens de lista
			if (normalizedMsg?.listMessage) {
				const listType = normalizedMsg.listMessage.listType
				const listTypeStr = listType === proto.Message.ListMessage.ListType.SINGLE_SELECT ? 'single_select' : 'product_list'
				;(stanza.content as BinaryNode[]).push({
					tag: 'biz',
					attrs: {},
					content: [{
				...
				logger.debug({ jid: destinationJid }, 'adding biz/list node for list message')
			}

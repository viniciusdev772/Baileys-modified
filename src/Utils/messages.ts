...
export const generateWAMessageContent = async (
		...
		switch (message.type) {
			...
			case 'interactive':
				m.interactiveResponseMessage = {
					...
					nativeFlowResponseMessage: message.buttonReply.nativeFlow
				}
				break
		}
	...
	// Process buttons if present (igual baileys-pro)
	if (hasOptionalProperty(message, 'buttons') && message.buttons?.length) {
		...
		if (hasNonNullishProperty(message, 'text')) {
			...
			buttonsMessage.headerType = (ButtonType as any)[type] ?? ButtonType.EMPTY
	...
	// Process interactive buttons (NativeFlow) - igual baileys-pro
	if (hasOptionalProperty(message, 'interactiveButtons') && message.interactiveButtons?.length) {
		...
		if (hasNonNullishProperty(message, 'text')) {
			...
			interactiveMessage.header = {
				...
				hasMediaAttachment: (message as any).media ?? false
			}
		} else if (hasOptionalProperty(message, 'caption')) {
	...
	// Process carousel - igual baileys-pro
	if (hasOptionalProperty(message, 'cards') && message.cards?.length) {
		const slides = await Promise.all(
			message.cards.map(async (slide: Carousel) => {
				...
				if (product) {
					...
					const mediaMsg = await prepareWAMessageMedia({ image }, options)
					header = { ...mediaMsg, title, hasMediaAttachment: true }
				} else if (video) {
					const mediaMsg = await prepareWAMessageMedia({ video }, options)
					header = { ...mediaMsg, title, hasMediaAttachment: true }
				}

				return {
					header: WAProto.Message.InteractiveMessage.Header.fromObject(header),
					body: WAProto.Message.InteractiveMessage.Body.fromObject({ text: caption }),
		...
		if (hasNonNullishProperty(message, 'text')) {
			interactiveMessage.body = WAProto.Message.InteractiveMessage.Body.fromObject({ text: message.text })
		}

		if (hasOptionalProperty(message, 'footer') && message.footer) {
			interactiveMessage.footer = WAProto.Message.InteractiveMessage.Footer.fromObject({ text: message.footer })
		}
	...
	// Process list message - igual baileys-pro
	if (hasOptionalProperty(message, 'sections') && message.sections?.length) {
		const listMessage = proto.Message.ListMessage.fromObject({
			...
			description: hasNonNullishProperty(message, 'text') ? message.text : undefined,
